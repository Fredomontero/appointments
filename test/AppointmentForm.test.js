...
describe('AppointmentForm', () => {
  ...
  it("Submits new value when submitted", async () => {
    expect.hasAssertions();
    render(
      <AppointmentForm
        service="Cut" 
  ...
  describe('time slot table', () => {
    ...
    it('renders a time slot for every half of hour between open and close times', () => {
    ...
    it.skip('preselects the existing value', () => {
      const today = new Date();
      const checkedTimeSlot = "09:00";
      const availableTimeSlots = [
        { startsAt: today.setHours(9, 0, 0, 0) },
        { startsAt: today.setHours(9, 30, 0, 0) }
      ];
      render(<AppointmentForm availableTimeSlots={availableTimeSlots} today={today} checkedTimeSlot={checkedTimeSlot} />);
      let checkedInput = startsAtField(0);
